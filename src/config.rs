...
impl Config {
    pub fn load(path: &str) -> Result<Self> {
        ...
        let key_url = config.get("url", "KEY").map_or("".to_string(), |s| s.to_string());
        ...
        let external_key_download = config.getuint("Download", "EXTERNAL_KEY").unwrap_or(Some(0)).unwrap_or(0) != 0;
        ...
        let tmp_key_folder_name = config.get("folder", "TMP_KEY_FOLDER_NAME").map_or("key_files".to_string(), |s| s.to_string());
    ...
    pub fn tmp_key_folder_path(&self) -> std::path::PathBuf {
        self.tmp_folder_path().join(&self.tmp_key_folder_name)
    }
